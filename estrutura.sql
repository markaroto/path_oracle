/*
#USAGE
# sqlplus teste@teste  @estrutura.sql "Local onde arquivo sera gerado"
*/
...
select 'hora=`date +%H:%M:%S`'|| chr(10)
	||'data_log=`date +%d-%m-%Y`'||chr(10)
	||'printf "%s%s%s%s%s%s%-10s%-20s%10d%s%-30s\n" ${data_log} " " ${hora} " " ${host1} " " ${banco} ${script_atual} ${PID} " - ${1}"  >> "/bkp/estrutura.log"' from dual;	
...
UNION
select 'Data_file' as name, substr(file_name,0,instr(file_name,'/',-1,1)-1)  as valor from dba_data_files
union
select 'Log_file' as name, substr(member,0,instr(member,'/',-1,1)-1) as valor from V$logfile
union
select 'TEMP_FILE' as name,substr(name,0,instr(name,'/',-1,1)-1) as valor from V$TEMPFILE
union
select name as name,'$ORACLE_HOME'||substr(value,instr(value,'?',-1,1)+1)  as valor from V$parameter where value like '%?/%'
union
select 'Directory '||directory_name as name,directory_path as value from DBA_DIRECTORIES
union
